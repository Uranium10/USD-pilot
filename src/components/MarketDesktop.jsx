  ...
  return <main className="desktop-shell">
    <div className="desktop-workspace">
      {activeApp === 'notepad' ? <InformationNotepad rumors={state.purchasedRumors} /> : <section className="desktop-window market-window">
        ...
        <section className="account-strip"><div className="asset-cell"><small>총자산</small><strong>{money(netWorth)}</strong>{previousSummary && <small className={assetChange >= 0 ? 'green' : 'red'}>전일 대비 {assetChange >= 0 ? '+' : ''}{money(assetChange)}</small>}{state.feedback && <div key={state.feedback.id} className={`asset-feedback ${state.feedback.amount >= 0 ? 'gain' : 'loss'}`}>{state.feedback.amount >= 0 ? '+' : ''}{money(state.feedback.amount)}</div>}</div><div><small>현금</small><strong>{money(state.cash)}</strong></div><div><small>총 부채</small><strong className="red">{money(state.debt)}</strong></div><div><small>이번 주 최소 상환</small><strong className="red">{money(minPayment)}</strong></div></section>
